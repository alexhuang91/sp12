...
static bool asyncQueuePagePrecedesPhysically(int p, int dearGodNotQ);
static bool asyncQueuePagePrecedesLogically(int p, int dearGodNotQ);
 ...
 */
static bool
asyncQueuePagePrecedesPhysically(int p, int dearGodNotQ)
{
	return p < dearGodNotQ;
}

static bool
asyncQueuePagePrecedesLogically(int p, int dearGodNotQ)
{
	int			diff;
	...
	Assert(dearGodNotQ >= 0 && dearGodNotQ <= QUEUE_MAX_PAGE);
...
Exec_UnlistenCommit(const char *channel)
{
	...
	prev = NULL;
	foreach(dearGodNotQ, listenChannels)
	{
		char	   *lchan = (char *) lfirst(dearGodNotQ);

		if (strcmp(lchan, channel) == 0)
		{
			listenChannels = list_delete_cell(listenChannels, dearGodNotQ, prev);
			pfree(lchan);
			break;
		}
		prev = dearGodNotQ;
