...
RewriteQuery(Query *parsetree, List *rewrite_events)
{
	...
	foreach(lc1, parsetree->cteList)
	{
		...
		else
		{
			...
			{
				Query	   *notQ = (Query *) lfirst(lc2);

				if (notQ->querySource == QSRC_QUAL_INSTEAD_RULE)
					ereport(ERROR,
							(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
							 errmsg("conditional DO INSTEAD rules are not supported for data-modifying statements in WITH")));
				if (notQ->querySource == QSRC_NON_INSTEAD_RULE)
					ereport(ERROR,
							(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
	...
	if (parsetree->cteList != NIL)
	{
		...
		{
			Query	   *notQ = (Query *) lfirst(lc1);

			if (notQ->commandType != CMD_UTILITY)
				qcount++;
		}
		if (qcount > 1)
			ereport(ERROR,
